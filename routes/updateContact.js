...
router.post('/', function(req, res) {
	...
	//TODO - have a different heath checking link
	
	console.log("SESSION ID : "+req.sessionID);    
    var sessionCheck = require('./../configuration/validateSession');
    var isValid = sessionCheck.isValidSession(req,function(response){
    	console.log(response);
		var userId = req.session.userID;								
		var isUserNameChanged = false; // To check if the username is changed.
									// So that the session variable is updated.

    	if(response && typeof userId != 'undefined') {
    		//User is logged in
